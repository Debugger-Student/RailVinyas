...
import LiveSectionPanel from "../components/LiveSectionPanel";
...
export default function NewBlockRequest() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      {/* HIGHLIGHTED HERO — the flagship feature of the app gets the
          featured treatment: gradient, glow, badge, elevated shadow.
          Every other page (Reports, Admin, plain lists) stays flat/plain
          by contrast, so this genuinely reads as "the special section." */}
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-navy via-navy to-[#0F557D] shadow-xl border border-sky/20">
        <div className="absolute -right-20 -top-20 h-64 w-64 bg-sky/15 rounded-full blur-3xl" />
        <div className="absolute -left-10 bottom-[-100px] h-56 w-56 bg-amber-400/10 rounded-full blur-3xl" />
        <div className="h-1 bg-gradient-to-r from-[#FF9933] via-white to-[#138808]" />
        <div className="relative px-7 py-6">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/10 border border-sky/30 px-3 py-1.5 text-[10px] uppercase tracking-[0.15em] font-bold text-sky">
            <span className="h-1.5 w-1.5 rounded-full bg-sky animate-pulse" />
            AI Optimization Engine · Live
          </span>
          <h1 className="text-2xl lg:text-3xl font-extrabold text-white mt-3">New Block Request</h1>
          <p className="text-sm text-white/60 mt-1.5 max-w-xl">
            Pick a section and the AI engine ranks all 24 candidate hours by real traffic, asset
            availability, and predicted overrun risk — live section context included below.
          </p>
        </div>
      </div>
      ...
      <Card>
        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {resolvedSection && (
            <div className="sm:col-span-2">
              <LiveSectionPanel
                sectionId={resolvedSection.section_id}
                fromName={resolvedSection.from_station_name}
                toName={resolvedSection.to_station_name}
              />
            </div>
          )}
